...
class ContextualBandit:
    ...
    def pull_arm(self, user_action):
        # Get a random number.
        bandit = self.bandits[self.state, user_action]
...
class Agent:
    def __init__(self, lr, s_size, a_size):
        ...
        state_in_one_hot = slim.one_hot_encoding(self.state_in, s_size)
        output = slim.fully_connected(state_in_one_hot, a_size,
        ...
        # 使用policy gradient方法进行训练
        # 实际上就是根据action的回报去奖励或者惩罚output
        # 这里只要奖励是正的就会奖励RL选中的哪个output，否则就会惩罚RL选中的output
        # 最终TF会通过跳转weight值达到上述目的
